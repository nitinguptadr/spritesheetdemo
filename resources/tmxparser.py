            ...
            for img in tile_set.images:
                if img.trans:
                    img.trans = (int(img.trans[:2], 16), \
                                 ...
                                 int(img.trans[4:], 16))
            for tileoffset in tile_set.tileoffsets:
                tileoffset.x = int(tileoffset.x)
                tileoffset.y = int(tileoffset.y)
...
class TileOffset(object):
    """
    A tileoffset holds the x, y pixel offset of the tiles when drawing them.
...
class TileMapParser(object):
    ...
    def _get_tile_set(self, tile_set_node, tile_set, base_path):
        ...
        for node in self._get_nodes(tile_set_node.childNodes, 'tileoffset'):
            self._build_tile_set_tileoffset(node, tile_set)
        self._set_attributes(tile_set_node, tile_set)
        return tile_set
    ...
    def _build_tile_set_tileoffset(self, tile_set_node, tile_set):
        tileoffset = TileOffset()
        attrs = tile_set_node.attributes
        for attr_name in list(attrs.keys()):
            setattr(tileoffset, attr_name, attrs.get(attr_name).nodeValue)

        tile_set.tileoffsets.append(tileoffset)
